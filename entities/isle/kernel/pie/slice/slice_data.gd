...
func _init(pie_: PieData) -> void:
	pie = pie_
	index = pie.slices.size()
	
	matter = Catalog.slice_matters[index]
	volume = Catalog.slice_volumes[index]
	pie.slices.append(self)
	
	if not pie.matter_to_volume_to_slice.has(matter):
		pie.matter_to_volume_to_slice[matter] = {}
	
	pie.matter_to_volume_to_slice[matter][volume] = self
	
	if not pie.volume_to_matter_to_slice.has(volume):
		pie.volume_to_matter_to_slice[volume] = {}
	
	pie.volume_to_matter_to_slice[volume][matter] = self
